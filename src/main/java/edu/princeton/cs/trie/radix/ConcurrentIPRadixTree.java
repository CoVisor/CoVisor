 ...
 */
package edu.princeton.cs.trie.radix;
...
import static edu.princeton.cs.trie.radix.ConcurrentIPRadixTree.SearchResult.Classification;
 ...
 */
public class ConcurrentIPRadixTree<O> implements IPRadixTree<O>, PrettyPrintable {
     ...
     */
    public ConcurrentIPRadixTree(NodeFactory nodeFactory) {
     ...
     * by other threads
     */
    public ConcurrentIPRadixTree(NodeFactory nodeFactory, boolean restrictConcurrency) {
    ...
    /*@Override
    public List<CharSequence> getPrefixKeysStartingWith(CharSequence prefix) {
        acquireReadLockIfNecessary();
        try {
        	
        	List<Node> prefixNodeList = prefixSearchTree(prefix);
        	List<CharSequence> prefixKeys = new ArrayList<CharSequence>();
        	int curLength = 0;
        	for (Node node : prefixNodeList) {
        		curLength += node.getIncomingEdge().length();
				if (node.getValue() != null) {
					prefixKeys.add(CharSequences.getPrefix(prefix, curLength).toString());
				}
        	}
        	return prefixKeys;
        }
	...
	@Override
    public List<O> getValuesForPrefixKeysStartingWith(CharSequence prefix) {
    	acquireReadLockIfNecessary();
        try {
        	
        	List<Node> prefixNodeList = prefixSearchTree(prefix);
        	List<O> prefixValues = new ArrayList<O>();
        	for (Node node : prefixNodeList) {
        		Object value = node.getValue();
				if (value != null) {
					prefixValues.add((O) value);
				}
        	}
        	return prefixValues;
        }
    ...
    @Override
    public List<KeyValuePair<O>> getIPKeyValuePairsForKeysStartingWith(CharSequence prefix) {
    	acquireReadLockIfNecessary();
        try {
        	
        	PrefixSearchResult searchResult = prefixSearchTree(prefix);
            List<KeyValuePair<O>> prefixKeyValues = new ArrayList<KeyValuePair<O>>();
            
            // get ascendant
            int curLength = 0;
            for (int i = 0; i < searchResult.prefixNodeList.size() - 1; i++) {
            	Node node = searchResult.prefixNodeList.get(i);
        		curLength += node.getIncomingEdge().length();
        		Object value = node.getValue();
				if (value != null) {
					prefixKeyValues.add(
							new KeyValuePairImpl<O>(
									CharSequences.getPrefix(prefix, curLength).toString(),
									(O) value));
				}
        	}
            ...
            switch (classification) {
                case EXACT_MATCH: {
                    Iterator<KeyValuePair<O>> iterator = getDescendantKeyValuePairs(prefix, searchResult.nodeFound).iterator();
                    while (iterator.hasNext()) {
                    	prefixKeyValues.add(iterator.next());
                    }
                    break;
                }
                case KEY_ENDS_MID_EDGE: 
     ...
     */
    @SuppressWarnings({"JavaDoc"})
    Iterable<KeyValuePair<O>> getDescendantKeyValuePairs(final CharSequence startKey, final Node startNode) {
    ...
    PrefixSearchResult prefixSearchTree(CharSequence key) {
    	List<Node> prefixNodeList = new ArrayList<Node>();
    	//prefixNodeList.add(root);
        ...
        return new PrefixSearchResult(key, currentNode, charsMatched, charsMatchedInNodeFound,
        		parentNode, parentNodesParent, prefixNodeList);
    }
    ...
    static class PrefixSearchResult extends SearchResult {
    	final List<Node> prefixNodeList;
    	
    	PrefixSearchResult(CharSequence key, Node nodeFound, int charsMatched, int charsMatchedInNodeFound,
    			Node parentNode, Node parentNodesParent, List<Node> prefixNodeList) {
    		super(key, nodeFound, charsMatched, charsMatchedInNodeFound, parentNode, parentNodesParent);
    		this.prefixNodeList = prefixNodeList;
    	}
